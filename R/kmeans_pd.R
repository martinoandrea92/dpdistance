...
dp.kmeans <- function( FD, n.cl = 2, metric, p = NULL) {
  ...
  while( sum( abs( cluster - cluster.old ) ) > 0) {
    ...
    if (metric == "L2") {
      for (i in 1:n) {
        for (k in 1:n.cl){
          Dist[i, k] <- ( funDist( grid, lapply( data, '[', i, TRUE ), lapply( C, '[', k, TRUE ), metric = "L2" ) )
        }
      }
    }
      ...
      for ( i in 1:n ) {
        for ( k in 1:n.cl ) {
          Dist[i, k] <- ( funDist( grid, lapply( data, '[', i, TRUE ), lapply( C, '[', k, TRUE ), metric = "trunc", lambda = lambda_hat, phi = phi_hat) )
        }
      }
    }
      ...
      for ( i in 1:n ) {
        for ( k in 1:n.cl ) {
          Dist[i, k] <- ( funDist( grid, lapply( data, '[', i, TRUE ), lapply( C, '[', k, TRUE ), metric = "mahalanobis", lambda = lambda_hat, phi = phi_hat, p = p) )
        }
      }
    }
